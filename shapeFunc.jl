...
#    Np = zeros(T2,4,4*nel); Bp = zeros(T2,8,4*nel)#p和d的维数相同,p=d
    Nu = zeros(T2,8,8*nel); Bu = zeros(T2,12,8*nel)
#    Bu_vol = zeros(T2,4,8*nel);Bu_xx = zeros(T2,4,8*nel);Bu_yy = zeros(T2,4,8*nel);Bu_xy = zeros(T2,4,8*nel);
    Ndp = zeros(T2,16,4*nel); Bdp = zeros(T2,16,4*nel)#d^2
#    Npp = zeros(T2,16,4*nel); Bpp = zeros(T2,16,4*nel)#p^2
...
#    Bu_vol[1:4,:] = Bu[1:3:10,:]+Bu[2:3:11,:]
#    Bu_xx = Bu[1:3:10,:];Bu_yy = Bu[2:3:11,:];Bu_xy = Bu[3:3:12,:];
#    Np = deepcopy(Nd);Bp = deepcopy(Bd)
#    Npp = deepcopy(Ndp);Bpp = deepcopy(Bdp)
#    return Nd,Ndp,Bd,Bdp,Nu,Bu,Bu_vol,Np,Bp,Npp,Bpp,detjacob,Bu_xx,Bu_yy,Bu_xy
    return Nd,Ndp,Bd,Bdp,Nu,Bu,detjacob
end
#5.自由度指标
function indices_fields() 
    nel=size(element,1);
    edofMat=zeros(Int,nel,8);
    for iel=1:nel
    ...
    iKu::Array{Int} = reshape(kron(edofMat,ones(8,1))',64*nel)
    jKu::Array{Int} = reshape(kron(edofMat,ones(1,8))',64*nel)
    #存储单元的d自由度指标
    dedofMat = element
    iKd::Array{Int} = reshape(kron(element,ones(4,1))',16*nel)
    jKd::Array{Int} = reshape(kron(element,ones(1,4))',16*nel)
    iFd::Array{Int} = reshape(element',4*nel)
...
@time begin
    Nd,Ndp,Bd,Bdp,Nu,Bu,detjacob = shapeFunc(node,element)
