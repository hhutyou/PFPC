function Hn1_comp!(Hn1::Array{T,2},εᵉ::Array{T,2}) where T<:Float64
    ...
    # ψᵉ[Mat_ind12] .= 1e19
    # #
    ψᵉ .= Kv0/2.0 .* (heaviside.(operator_tr(εᵉ))).^2 .- μ0/3.0 .* (operator_tr(εᵉ)).^2 .+
        μ0*((εᵉ[1,:]).^2 .+ (εᵉ[2,:]).^2 .+ (εᵉ[3,:]).^2 .+ 0.5.*(εᵉ[4,:]).^2 .+ 0.5.*(εᵉ[5,:]).^2 .+ 0.5.*(εᵉ[6,:]).^2)
    ...
    # D1 = zeros(size(ψᵉ))
    Hn1::Array{T,2} = max.(reshape(D1,8,nel),Hn1)
    return Hn1
end
##
