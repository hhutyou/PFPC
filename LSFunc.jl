...
function LSFunc(LS::T,δu::Array{T},u_old::Array{T},εᵖ::Array{T},d1::Array{T},dg1::Array{T},H0::Array{T},Hn1::Array{T},εᵖ_com::Array{T},f_ext) where T<:Float64
    ...
    # idxTen=findall(σ₁.>f0p)##拉应力状态
    # idxCom=findall(σ₁.<=f0p)##压应力状态
    if isempty(idxTen)==0  ##拉
        εᵖ[:,idxTen] .= kron(1.0 .- m0.(dg1[idxTen]), ones(1,3))' .* (ε[:,idxTen])
    end
    δεᵖ .= plasticity!(σc,dg1,idxCom)
    ...
    @sync @distributed for iel=1:nel
        ...
        # sparse(reshape(DK[:,4*(iel-1)+3],3,3)),sparse(reshape(DK[:,4*(iel-1)+4],3,3)))*
        # reshape(εᵖ[:,4*(iel-1)+1:4*iel],12,1)
        # sigma[:,4*(iel-1)+1:4*iel] = reshape(σ_t[:,iel],3,:) .+ kron((1.0 .-dg1[4*(iel-1)+1:4*iel]).^2 .- 1.0, ones(1,3))'.*
        #     sigma_plus(εᵉ, collect(4*(iel-1)+1:4*iel), λ[4*(iel-1)+1:4*iel],G[4*(iel-1)+1:4*iel], "plane-strain")
    end
    σ .= reshape(sdata(σᵗ),3,4*nel)
    # εᵉ = ε - εᵖ
    ...
    # end
    # σ .= reshape(sdata(σᵗ),3,4*nel)
    # idxTen0 = setdiff(idxTen,Mat_ind0)
    # idxTen12 = setdiff(idxTen,Mat_ind12)
    # σ[:,idxTen0] = reshape(sdata(σᵗ),3,4*nel)[:,idxTen0] .+ kron((1.0 .-dg1[idxTen0]).^2 .- 1.0, ones(1,3))' .*sigma_plus(εᵉ,idxTen0,λ1,G1,"plane-strain")
    # σ[:,idxTen12] = reshape(sdata(σᵗ),3,4*nel)[:,idxTen12] .+ kron((1.0 .-dg1[idxTen12]).^2 .- 1.0, ones(1,3))' .*sigma_plus(εᵉ,idxTen12,λ2,G2,"plane-strain")
    # σ[:,Mat_ind0] = reshape(sdata(σᵗ),3,4*nel)[:,Mat_ind0] .+ kron((1.0 .-dg1[Mat_ind0]).^2 .- 1.0, ones(1,3))' .*sigma_plus(εᵉ,Mat_ind0,λ1,G1,"plane-strain")
    # σ[:,Mat_ind12] = reshape(sdata(σᵗ),3,4*nel)[:,Mat_ind12] .+ kron((1.0 .-dg1[Mat_ind12]).^2 .- 1.0, ones(1,3))' .*sigma_plus(εᵉ,Mat_ind12,λ2,G2,"plane-strain")
    # σ[:,idxTF] = reshape(sdata(σᵗ),3,4*nel)[:,idxTF] .+ kron((1.0 .-dg1[idxTF]).^2 .- 1.0, ones(1,3))' .*sigma_plus(εᵉ[:,idxTF],λ[idxTF],G[idxTF],"plane-strain")
    f_int = FMat(node,element,Bu,detjacob,σ,edofMat)
    # S = (f_ext-f_int)'*δu
    S = norm(f_ext[freedofs] .- f_int[freedofs])
