...
function crack_orientation(d1::Array{T2,1},cond_inter::Array{T1,1}, element_central::Array{T2,2}) where {T1<:Int,T2<:Float64}
    #01检索裂纹路径Γfinal______________________________________________________________________________________________________________
    ...
    while size(Γtmp, 1) != 0
        ...
        for i = 1:size(Γtmp_id, 1)
            if (node[Γtmp_id[i], 1] - node[N1, 1])^2 + (node[Γtmp_id[i], 2] - node[N1, 2])^2 < (5*mesh_size)^2
...
#=     #02_1 spline
    ...
    end_point = Array{Float64,2}(undef, size(Γfinal_sort, 1), 2)
    end_point = node[Γfinal_sort, :] #终点坐标（已排序） 
    length2 = Array{Float64,1}(undef, size(Γfinal_sort, 1))
