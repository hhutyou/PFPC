function integration_d(Hn1::Array{T},d1::Array{T},dg1::Array{T}) where T<:Float64 ##
    # Hn = Array{Float64,2}(undef,size(Hn1))
    # d3 = zeros(nnode)
    # d1 = zeros(nnode) .+ d0/2
    # d2 = zeros(nnode) .+ d0
    # dg1_t=SharedArray{Float64,1}(4*nel)
    # dg2_t=SharedArray{Float64,1}(4*nel)
    # @sync @distributed for iel = 1:nel
    #     dg1_t[4*(iel-1)+1:4*iel]=Nd[:,4*(iel-1)+1:4*iel]*d1[dedofMat[iel,:]]
    #     dg2_t[4*(iel-1)+1:4*iel]=Nd[:,4*(iel-1)+1:4*iel]*d2[dedofMat[iel,:]]
    # end
    # dg1 = sdata(dg1_t)
    # dg2 = sdata(dg2_t)
    #
